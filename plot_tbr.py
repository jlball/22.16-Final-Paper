...
ax.scatter(percents, tbr_plut, label=r'$UF_4$', s=20, c='tab:green')
ax.scatter(percents, tbr_th, label=r'$ThF_4$', s=20, c='tab:red', marker='x')
#ax.plot(fit_x, fit_y)

ax.set_yscale('linear')
ax.set_xlabel(r"Percent $UF_4$ or $ThF_4$ by volume", fontsize=14)
ax.set_ylabel(r"Tritium Breeding Ratio (TBR)", fontsize=14)
ax.set_title(r"TBR vs. Volume Percent of $UF_4$ and $ThF_4$ Dopants in a FLiBe Blanket with an ARC-like Geometry", fontsize=12)

ax.legend()

#plt.hlines(24*365*0.8, 0, 6, colors=['tab:orange'], linestyles="dashed")

plt.hlines(1, 0, 6, colors=['tab:blue'], linestyles="dashed")
